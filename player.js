  ...
  this.step = function() {
    if (this.active) {
      ...
      for (let i = 0; i < interactives.length; i++) {
        console.log(
          interactives[1].active + " " + interactives[1].type + player.y
        );
        let interactiveRect = {
          x: interactives[i].x,
          y: interactives[i].y,
        ...
        if (checkIntersection(verticalRect, interactiveRect)) {
          this.yspeed = 0;
          if (interactiveRect.type == 1) {
            interactiveRect.interactive.active = true;
          }
        } else {
          if (interactiveRect.type == 1) {
            interactiveRect.interactive.active = false;
          }
        }
      }
